...
> `init-db` followed by `alembic stamp head`. An existing database
...
1. `init-db` creates the current schema and reconciles roles, permissions,
   subject types, and the bootstrap administrator.
...
3. `alembic current` confirms that the database is tracked at the current head.
...
`clean-demo` is destructive. Take a verified backup first.
