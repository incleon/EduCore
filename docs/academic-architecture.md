...
For a new empty database, follow the fresh-install sequence in
[`database-setup.md`](database-setup.md) so the legacy baseline is stamped only
after the current schema has been created.
